...
class TempestExtremesMCS(TempestExtremesAbstract):
    """
    ...
    def run(self, *args, **kwargs):
        ...
        for track_type in self.mcs_types:
            ...
            for do_track_file in dot_tracking_files:
                ...
                for regrid_resol in self.regrid_resolutions:
                    ...
                    if glob.glob(file_search):
                        ...
                        self.processed_files_psl[regrid_resol] = \
                            self.processed_files[ftimestamp_day]["pr"]
                        self.variable_units = variable_units

                        # run TempestExtremes detect blobs
                        candidate_file = self._run_detect_blobs(ftimestamp_day)

                        # process the MCS netcdf files
                        self._process_mcs_for_archive(candidate_file)
        ...
        if is_new_year:
            self._produce_annual_mcs_file(self.outdir, timestamp_previous[0:4])

        if self.is_last_cycle == "true" or is_new_year:
            # archive any remaining MCS data
            self._archive_mcs_data(self.outdir, is_new_year, timestamp_previous[0:4])
    ...
    def _run_detect_blobs(self, timestamp):
        ...
        for mcs_type in self.mcs_types:
            self.logger.debug(f"Running {mcs_type} detection")
            candidatefile = os.path.join(
                self.outdir,
                f"{self.runid}_MCSmask_{timestamp}_{mcs_type}.nc",
            )
            self.logger.debug(f"candidatefile {candidatefile}")

            #cmd_io = '{} --out {} '.format(
            #        self.mcs_detectblobs_script,
            #        candidatefile)
            cmd_io = "{} ".format(self.mcs_detectblobs_script)
            ...
            tracking_phase_commands = self._construct_command(mcs_type)
            cmd_detectblobs = cmd_io + tracking_phase_commands["detectblobs"]
            self.cmd_detect_type[mcs_type] = cmd_detectblobs
    ...
    def _process_mcs_for_archive(
        ...
        """
        Process any MCS files before archiving
    ...
    def _produce_annual_mcs_file(
        self,
        outdir,
        year,
    ):
        """
        Concatenate one year of MCS data
        :param str outdir: output directory which contains MCS files
        :param str year: the year of data to concatenate
        """
        files_to_join = sorted(glob.glob(os.path.join(outdir, "*MCSmask_"+year+"????_*.nc")))
    ...
    def _archive_mcs_data(self, outdir, is_new_year, year):
        """
        Archive the required MCS data
        :param str outdir: output directory from which to archive MCS files
        """
        if self.is_last_cycle == "true" :
            files_to_archive = glob.glob(os.path.join(outdir, "*MCSmask*.nc"))
        elif is_new_year:
            files_to_archive = glob.glob(os.path.join(outdir, "*MCSmask*year_"+year+"*.nc"))
        ...
        if len(files_to_archive) > 0:
            for mcs_file in files_to_archive:
                # move mcs_file to archive directory
                mcs_archive_file = os.path.join(outdir, self._archived_files_dir,
                                                os.path.basename(mcs_file))
                if self.is_last_cycle:
                    shutil.copy(mcs_file, mcs_archive_file)
                else:
                    os.replace(mcs_file, mcs_archive_file)
                    os.remove(mcs_file)

                with open(mcs_archive_file + ".arch", "a"):
                    os.utime(mcs_archive_file + ".arch", None)
    ...
    def _get_app_options(self):
        ...
        self.mcs_detectblobs_script = self.app_config.get_property(
            "common", "mcs_detectblobs_script"
        )
        # track_types is a Python list so eval converts str to list
        self.mcs_types = eval(self.app_config.get_property("common", "mcs_types"))
